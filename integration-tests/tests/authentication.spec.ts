...
test('Entering website unauthenticated will redirect to login page', async ({ page }) => {
    await page.goto('http://localhost:3000');

    await page.waitForURL('http://localhost:3000/login');
});

test('Example student can log in with his email and password', async ({ page }) => {
...
test('New user can register and will be redirected to the home page', async ({ page }) => {
